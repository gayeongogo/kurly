...
class KurlyComponent extends Component {
    ...
    //2. localStorage 데이터 전체 추출
    //3. 아이디만 추출
    //4. 반복처리 후 비교하여 같은 아이디가 있다면 true로 변환
    //5. 결과 true면 '이미 등록된 아이디 입니다.' / false면 '사용가능한 아이디 입니다.'
    idDoubleCheck(value){
        const inputId=value;
        ...
        for(let i=0; i<localStorage.length; i++){
            /* if(imsiArr.push(JSON.parse(localStorage.getItem(localStorage.key(i))).아이디 === inputId)){
                ok=true;//객체에 이 조건 안들어옴
                   
            } */
            imsiArr.push(JSON.parse(localStorage.getItem(localStorage.key(i))).아이디)//수정(밖으로 빼고 조건식 다시 씀)
    ...
    onSubmitEvent=(e)=>{
        ...
        const { 
            아이디, 비밀번호, 이름, 이메일, 휴대폰,  주소1, 주소2, 주소, 성별, 생년, 생월, 생일, 추가입력사항, 약관동의, 아이디ok, 비밀번호1ok, 비밀번호2ok, 비밀번호3ok, 이메일ok, 휴대폰ok, 아이디중복확인, 추가입력상자
        } = this.state;      
                            ...
                            <li>
                                <div className="gap">
                                    ...
                                    <div className="input-box">
                                        ...
                                        <button disibled={this.disibled1} onClick={this.onClickPhoneEvent} className={`w120-btn ${this.state.isPhoneClass ? '': 'phone'}`}>인증번호받기</button>
                                        {
                                            this.state.isPhoneOkOpen && (
                                                <>
                                                <input type="text" maxLength='6' className='inputText' id='inputPhone1' placeholder='인증번호를 입력하세요' value={this.state.휴대폰인증} disibled={this.disibled2} onChange={(e) => this.onChangePhoneOk(e.target.value)} />
                                                <button className={`w120-btn phone phoneOk-btn ${this.state.isPhoneOkClass ? '' : 'phone'}`} disibled={this.disibled2} onClick={this.onClickPhoneOkEvent}>인증번호확인</button>
                                                {
                                            ...
                                            </div>
                                            {
                                                this.state.isAddInputBoxShow && (
                                                    <div className="add-input-box">
                                                        <input type="text" id='addInputBox' placeholder={this.state.addInputPlaceHolder} value={this.state.추가입력상자} onChange={(e) => this.onChangeAddInput(e.target.value)} />
                                                        <p>
